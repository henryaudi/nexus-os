...
void encodeGdtEntry(uint8_t *target, struct gdt_structured source)
{
    ...
    {
        panic("encodeGdtEntry: Invalid GDT limit\n");
    }

    target[6] = 0x40; // Set the default flags for the GDT entry
    if (source.limit > 65536)
    {
        source.limit = source.limit >> 12;
        target[6]    = 0xC0; // If the limit is greater than 64KB, set the granularity flag
    }
