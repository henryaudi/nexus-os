...
#define NEXUS_FAT16_FAT_ENTRY_SIZE 0x02
#define NEXUS_FAT16_BAD_SECTOR     0xFF7
#define NEXUS_FAT16_UNUSED         0x00
...
struct filesystem fat16_fs = {
    .resolve = fat16_resolve, .open = fat16_open, .read = fat16_read, .seek = fat16_seek};
...
int fat16_get_total_items_for_directory(struct disk *disk, uint32_t directory_start_sector)
{
    ...
    if (diskstreamer_seek(stream, directory_start_pos) != NEXUS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }

    // Sync successfully at the directory start position, now we can read the directory items until
    // we hit an empty item.
    while (1)
    {
        if (diskstreamer_read(stream, &item, sizeof(item)) != NEXUS_ALL_OK)
        {
            res = -EIO;
            goto out;
        }
                             ...
                             struct fat_directory *directory)
{
    ...
    if (diskstreamer_read(stream, dir, root_dir_size) != NEXUS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
int fat16_resolve(struct disk *disk)
{
    ...
    if (fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != NEXUS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
static int fat16_get_fat_entry(struct disk *disk, int cluster)
{
    ...
    res = diskstreamer_seek(stream, fat_table_position * (cluster * NEXUS_FAT16_FAT_ENTRY_SIZE));
    if (res < 0)
    {
        goto out;
    }
...
/**
 * @brief Gets the correct cluster to use based on the starting cluster and offset
 */
static int fat16_get_cluster_for_offset(struct disk *disk, int starting_cluster, int offset)
{
    ...
    for (int i = 0; i < clusters_ahead; i++)
    {
        ...
        if (entry == NEXUS_FAT16_BAD_SECTOR)
        {
            res = -EIO;
            goto out;
        }
...
static int fat16_read_internal(struct disk *disk, int starting_cluster, int offset, int total,
                               void *out)
{
                                              ...
                                              const char *name)
{
    ...
    char             tmp_filename[NEXUS_MAX_PATH];
    for (int i = 0; i < directory->total; i++)
    {
        fat16_get_full_relative_filename(&directory->item[i], tmp_filename, sizeof(tmp_filename));
        if (istrncmp(tmp_filename, name, sizeof(tmp_filename)) == 0)
        {
            // Found the item, now we need to determine if it's a file or directory.
